...
from app.services.self_employed_upload_import_service import (
    import_self_employed_upload_via_staging_copy,
)
...
def _run_job(job_id: str, df, source_file_name: str):
    ...
    try:
        ...
        result = import_self_employed_upload_via_staging_copy(
            db,
            df,
            source_file_name=source_file_name,
...
@router.post("/upload-self-employed")
def upload_self_employed(
    ...
    """Upload SELF_EMPLOYED special Excel/CSV.
    ...
    - applications.application_number matches apprefno, and
    - application_electrical_installation.experience_type = 'SELF_EMPLOYED'

    Staging table: public.stage_self_employed_upload_raw
    """
    ...
    return JSONResponse(
        ...
        content={
            "status": "ACCEPTED",
            "job_id": job_id,
            "message": "Import started in background. Check status at /api/v1/electrical-installations/self-employed-status/{job_id}",
        },
    )


@router.get("/self-employed-status/{job_id}")
def get_self_employed_job_status(job_id: str):
    if job_id not in _job_status:
        raise HTTPException(status_code=404, detail="Job not found")
    return _job_status[job_id]


@router.get("/self-employed-jobs")
def list_self_employed_jobs():
    return _job_status
